...
- Loot the horde's monsters dropped that is still on the ground ten minutes after the horde ends disappears. Loot from anything else is left alone.
...
|---|---|
| Droops | drooplets, droops, spitters, the swarm, the empowered |
...
The monsters are the game's own, at the game's own strength, whatever level the defenders are. Droops are the weakest (a drooplet has about 20 health, a droop about 65). A goblin ripper has about 120, the later goblins about 1,000 to 3,000, a wolf about 320 and the alpha about 3,400. Pick the theme for your group; `Random` takes any of the four.

With fewer than five waves the mod keeps the first and the last row and spreads the rest; with more it repeats the last row, harder each time. `CustomWaves` replaces the theme with your own list, by the game's monster names: `GoblinRipper x4, GoblinFlaskrat x2 | GoblinBasher x3 | GoblinBasherT2 x1`. A name the game does not know is skipped with one line in the log. Monsters of different kinds may fight each other as they do in the game (a corrupted monster and a goblin do), and the game credits such a kill to the nearest player. A theme named on `/horde start` wins over `CustomWaves`.
...
| `Cleanup` | `DropsAfterMinutes` | Loot and gold from horde monsters that nobody picked up is removed this long after the horde ends, 0 to 30. 0 = leave it to the game, which removes a drop after 30 minutes | 10 |
...
That is about 3 percent of a core per living horde monster. `MaxAlive` is the hard cap on how many the mod keeps alive at once, whatever the wave size and the number of defenders, and a whole horde never makes more than 300. The default of 10 adds about a third of a core for as long as the horde lasts. On a server that shares its machine with others, keep it near the default.

Loot that nobody picks up costs CPU too, for as long as it lies there: about 0.8 percent of a core per item on the ground, measured on the same host (85% with two players and no drops, 95% with 12 items left by one default horde, 104% with 23, 113% with 35 after three hordes). That is the game's own cost of a ground item, whoever dropped it. `DropsAfterMinutes` exists because of it: with the default of 10 the same host read 96% before a horde, 115% after it and 95% once the horde's leftovers were removed. A server restart clears every ground drop as well.
...
The game ships a wave engine (`ArenaManager`) for one arena, but no manager object exists in the world, so the mod runs its own loop in that engine's shape. Monsters are made by `SceneHandler.SpawnMonsterInstance`, the runtime spawner the game's wave engine, summons and spawn spells call, with its spawn effect, its auto-aggro and its transient flag on; a wave's health and damage go through `Monster.ApplySpawnStatProfile` in the spawner's own configure step, as the engine does. Spawn points are sampled on the area's walk mesh in a ring round a living defender, and only a point with a clear walk line to that defender is used. Each monster is handed to the game's own aggro call (`SceneHandler.AggroMonster`). Some monsters of the game wait for their prey to come to them (wolves and droops do, goblins do not): a horde monster that stands still away from every defender is walked over with the game's escort behaviour (`MonsterBehaviour.SetEscortTarget`) and released into the fight when it arrives; one that still does not move is put down a few steps from the defender. A monster is counted dead when the game says it is, its corpse is removed after 25 seconds, and the end of a horde despawns whatever is left. A monster that a horde monster's spawn spell makes (`BaseSpellLibrary.SpawnMonster` with a horde monster as the caster) joins the horde and is removed with it, which is why a wave is cleared four seconds after its last death and not at it; a player's own summon is never touched. No monster of the four themes casts such a spell, so this has not been seen working. The loot a horde monster drops is noted inside that monster's own drop checks (`MonsterUtils.GoldDropCheck`, `RuneDropCheck`, `ItemDropCheck`, the three calls the game's death routine makes), together with the mod's own gold piles, and what is still on the ground `DropsAfterMinutes` after the horde ends is removed the way the game removes a drop on a pick-up or when its 30 minutes are up: the drop's object is destroyed on the server. A drop from anything else is never on the list. The list is kept in memory: after a server restart the game's own 30 minutes apply. Gold goes through the game's world-gold call (`ServerRPC.AddGoldToWorldServerRpc`), so whoever picks a pile up is paid by the game; healing sets health, concentration and stamina to their maximum. The top slayer is read after `QuestManager.RegisterMonsterDeath`, the game's own kill credit. Chat commands are read in a prefix on `ChatSystem.SendMessageToServerServerRpc`. Nothing is written to a character or to the world save, and no XP is granted outside the game's own kill path.

## Hordes and the world's story

The game ties its story to the kind of monster, not to the one the story placed. On a fresh world a horde's copy of the monstrous droop, the first quest's boss, called the camp invasion in when its health fell, exactly as the real one does. So the mod keeps its monsters out of the story: while the game runs a horde monster's death, one of its reactions or one of its boss stages, the game's call that starts a story event is skipped (one line in the log says which event); the death of a horde leader (a monster the game ranks mini-boss or boss) gives no quest credit; no horde monster's death is reported to the game's events. Ordinary horde monsters count for "slay N of X" quest steps and bounties, for the bestiary and for kill statistics like any other monster of their kind. If a game update takes one of those guards away, leaders and monsters that carry a story event are left out of hordes, with one line saying so. The four themes use no monster that carries a Steam achievement of its own; a custom wave may (the monstrous droop is the Droop King of `DEFEAT_DROOP_KING`), and killing it in a horde counts for that achievement.
...
Tested on game build 25350646 with WaygateServer 0.3.9, with headless test clients: monsters of all four themes spawn and are seen by clients, monsters and players kill each other with the game's own XP and gold paid, the cap on living monsters holds, every ending removes every horde monster, a server ended in the middle of a horde starts again without them, a horde's copy of a story boss starts no story event, and after the clean-up the area holds the monsters and the loot it held before the horde, a player's own summon and a player's own dropped item included. Not yet looked at on a real player's screen.
